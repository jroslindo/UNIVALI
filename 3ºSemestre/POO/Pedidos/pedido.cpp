...
Pedido::Pedido(string dataPedido, Cliente cliente, string localDeEntrega)
{
...
string Pedido::getLocalDeEntrega() const
{
...
string Pedido::getDataPedido() const
{
    return dataPedido;
}
...
void Pedido::imprimirPedido(){
    ...
    cout << "\nLocal de Entrega: " << this->localDeEntrega;
        if(this->faturado){
            cout <<"\nFaturado: Sim";
        }else{
            cout <<"\nFaturado: Nao";
    ...
    for(int i=0; i<this->qtdItens;i++){
        float preco = itens[i].getPrecoUnit()*itens[i].getQtd();
        total+=preco;
        cout << "\n [";
        cout <<i<<"\t"<<itens[i].getDescricao()
             <<"\t"<<itens[i].getQtd()
